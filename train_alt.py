...
def train_solo(run_id, env_path, config_path, total_runs=5, log_name=None, alternate_env=False):
    ...
    for i in range(total_runs):
        ...
        # Handle alternating environments
        if alternate_env:
            # Use actual run number (current_run_num) to determine environment
            if current_run_num % 2 == 1:  # Odd run numbers use NormalTrain
                actual_env_path = env_path.replace("AlternateTrain", "NormalTrain")
                env_type = "NormalTrain"
            else:  # Even run numbers use FogTrain
                actual_env_path = env_path.replace("AlternateTrain", "FogTrain")
                env_type = "FogTrain"
            print(f"Starting training: {current} (Run #{current_run_num} - {env_type})")
        else:
        ...
        sa = os.path.join(actual_env_path,
        ...
        cmd = [
            "mlagents-learn",
            config_path,
            "--env", str(Path(actual_env_path) / "2D go to target v1.exe"),
            "--run-id", current,
            "--force",
        ...
        if alternate_env:
            print(f"Completed training: {current} (Run #{current_run_num} - {env_type})")
        else:
...
def train_multiple_networks(networks, env_path, runs_per_network=2, log_name=None, env='NormalTrain'):
    ...
    # Check if we're using alternating environments
    is_alternate = env == 'AlternateTrain'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", type=str, default="NormalTrain",
                        help="Folder name under Builds/ to use as env (use 'AlternateTrain' to alternate between NormalTrain and FogTrain)")
    ...
    # Handle alternating environment setup
    if args.env == "AlternateTrain":
        # For alternating, we'll pass the base path but the train_solo function will handle switching
        env_folder = f"./Builds/AlternateTrain"  # This will be replaced dynamically
        print("🔄 ALTERNATING ENVIRONMENT MODE:")
        print("   Odd run numbers  → NormalTrain environment")
        print("   Even run numbers → FogTrain environment")
    else:
